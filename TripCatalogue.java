...
public class TripCatalogue {
	...
	public TripCatalogue() {
		...
		// name, length, location, time, seat,price, theme, handicapped,
		// difficulty, suitablekids, pricekids
		...
		Trip trip3 = new Trip("Blue Lagoon", 6, 2, 2, 40, 10000, 1, 1, 1, 1, 10000, 3);
	...
	public ArrayList<Trip> selectLocation(int loc) {
		...
		public ArrayList<Trip> selectID(int id) {
			ArrayList<Trip> results = new ArrayList<Trip>();

			for (int i = 0; i < trips.size(); i++) {
				if (id == (trips.get(i).getTripId())) {
					results.add(trips.get(i));
				}
			}
			return results;
